...
const CompanionGuide = () => {
  ...
  const defaultIntro =
    "Hey hey! I’m Kai, your cool MVP guide. Got a question for me? Pick a topic and I’ll help!";
  ...
  return (
    <>
      {!isOpen && (
        <div className="fixed bottom-6 right-6 z-50 group font-dm">
          <button
            ...
            aria-label="Open Kai's Companion Guide"
            className="relative w-[115px] h-[115px] cursor-pointer"
          >
            ...
            <Image
              src="/assets/RainbowRectangle.png"
              alt=""
              width={115}
              height={115}
            ...
            />
            <Image
              src="/assets/Visor.png"
              alt=""
              width={80}
              height={80}
              className="absolute -top-4 -right-9 rotate-25"
              unoptimized
            />
            <Image
              src="/assets/KaiHead2.png"
              alt="Kai avatar"
              width={90}
              height={90}
              className="absolute top-0 left-0 w-[110px] h-[110px] object-contain"
              unoptimized
            />
            {hasUnreadAnswer && (
              <span className="absolute -bottom-1 -left-1 w-5 h-5 rounded-full bg-red-500 text-white text-xs font-bold flex items-center justify-center shadow-md">
                1
              </span>
            )}
          </button>
      ...
      {isOpen && (
        <aside
          className="fixed bottom-6 right-12 w-[370px] rounded-[28px] bg-[#1e1e1e] text-white p-5 pb-0 z-50 border-6 border-black shadow-[8px_8px_0px_#000000]"
          role="dialog"
          aria-label="Kai Companion Guide"
        >
          <div className="flex items-center justify-between -mb-2">
            ...
            {!selectedTopic ? (
              ...
              <div
                className={`flex items-center mb-4 px-3 py-1 border-white border-5 shadow-[4px_4px_0px_#FFFFFF] rounded-[10px] font-bold text-white text-sm ${colorMap[selectedTopic.title]}`}
              >
          ...
          {!selectedQuestion && (
            <div className="flex items-start mb-5">
              <Image
                src="/assets/PurpleChatBubble.png"
                alt=""
                width={38}
                height={38}
                ...
                unoptimized
              />
              <p className="text-[12px] font-semibold leading-snug font-dm whitespace-pre-line">
                {typedIntro}
              </p>
            </div>
          )}
          {!selectedTopic && (
            <div
              className="grid grid-cols-2 gap-3 text-sm font-semibold"
              role="list"
            >
              ...
              <div className="gap-3 mb-12 grid grid-cols-2">
                {selectedTopic.questions.map((q, i) => (
                  <button
                    ...
                    className={`${colorMap[selectedTopic.title]} ${textColorMap[selectedTopic.title]} cursor-pointer w-full py-2 rounded-lg text-left text-sm shadow hover:brightness-90 transition flex items-center font-semibold`}
                  >
                    <Image
                      src={questionMark}
                      alt=""
          ...
          {selectedQuestion && (
            <>
              <div className="flex items-start mb-3">
                ...
                />
                <div
                  className={`${colorMap[selectedTopic.title]} ${textColorMap[selectedTopic.title]} px-3 py-2 rounded-lg text-sm shadow flex items-center font-semibold mb-2`}
                >
                  <Image
                    src={questionMark}
                    alt=""
              ...
              <div
                className={`${colorMap[selectedTopic.title]} ${textColorMap[selectedTopic.title]} p-4 rounded-lg mb-12 text-sm flex items-start shadow font-semibold`}
              >
                <Image
                  src={exclamationMark}
                  alt=""
                  width={24}
                  height={24}
                  className="mr-2 mt-1 object-contain"
                  unoptimized
                />
                <p>{typedAnswer}</p>
              </div>
          ...
          <div
            className="-bottom-4 left-72 w-[72px] h-[72px] relative cursor-pointer"
            onClick={handleToggle}
          >
            <Image
              src="/assets/RainbowRectangle.png"
              alt=""
              fill
              className="absolute top-0 left-0 w-full h-full shadow-[6px_6px_0px_#000000] rounded-[10px]"
              unoptimized
            />
            <Image
              src="/assets/KaiHead2.png"
              alt="Kai close button"
              fill
              className="absolute flex top-1 left-1 w-[64px] h-[64px] object-contain"
              unoptimized
            />
          </div>
        </aside>
